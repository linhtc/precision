...
Class Mongo_db
{
	 ...
	 *  @return  array|integer
	 */
	private function _get(string $collection = '', array $options = [], bool $get_count = FALSE, string $method = '', bool $one = false) // : array | int
	{
		if ($collection == '')
		{
...
// 				'readConcern' => $read_concern
			...
			if ($cursor instanceof Cursor)
			{
				...
				elseif($one){ /* Return first document object if true */
					foreach($array as $document){
						return $this->document_to_object($document);
					}
				}
	 ...
	 *  @return   array
	 */
	public function getOne(string $collection = '', array $options = []): stdClass
	{
		return $this->_get($collection, ['limit' => 1] + $options, FALSE, '', true);
	}
	 ...
	 *  @return   array
	 */
	public function getOneWhere(string $collection = '', array $where = [], array $options = []): stdClass
	{
		return $this->where($where)->_get($collection, ['limit' => 1] + $options, FALSE, '', true);
	}
